...
import './style.css';
import './readout.css';
import {mountReadout,type ReadoutController} from './readout-ui';
import {createReadoutVisual,type ReadoutVisual} from './readout-visual';
...
let mode = 4, selected = 0, separation = .48, stage = 0, playing = false, elapsed = 0;
...
let readout:ReadoutController|undefined;let readoutVisual:ReadoutVisual|undefined;
const fullPackage=()=>mode===0||mode===3||mode===4;
...
function rebuild(){
 model.traverse(o=>{if(o instanceof THREE.Sprite){o.material.map?.dispose();o.material.dispose();}if(o instanceof THREE.Mesh){o.geometry.dispose(); const mats=Array.isArray(o.material)?o.material:[o.material];mats.forEach(m=>m.dispose());}});
 readoutVisual=undefined;packageRig=undefined;model.clear(); groups=layers.map((_,i)=>{const g=new THREE.Group();g.userData.layer=i;model.add(g);return g;});
 ...
 if(mode===4){readoutVisual=createReadoutVisual(groups[3]);if(readout)readoutVisual.update(readout.getState());}
 positionLayers(); highlight();
}
function positionLayers(){
 const base=fullPackage()?[1.62,1.43,1.07,.75,-.1]:[1.04,.86,.51,.12,-.25];
 groups.forEach((g,i)=>{g.position.y=base[i]+separation*(4-i)*.9-1;g.visible=visibility[i]&&!((mode===3||mode===4)&&i<3);});
...
function highlight(){groups.forEach((g,i)=>g.traverse(o=>{if(o instanceof THREE.Mesh && o.material instanceof THREE.MeshStandardMaterial){const active=mode!==4&&(selectedPart?o.userData.part===selectedPart:i===selected);o.material.emissive.set(active?'#679652':'#000000');o.material.emissiveIntensity=active?.48:0;}}));}
...
function resetCamera(){camera.zoom=mode===4?1.25:1;camera.updateProjectionMatrix();camera.position.set(mode===4?5.8:fullPackage()?13:mode===2?8:10,mode===4?13.8:fullPackage()?14:10,mode===4?9:fullPackage()?17:mode===2?10:13);controls?.target.set(0,.5,0);controls?.update();}
function setMode(i:number){mode=i;playing=false;$('play').textContent='▶ 동작 재생';document.body.classList.toggle('readout-mode',i===4);for(const id of ['readout-controls','readout-analysis','readout-detail'])$(id).hidden=i!==4;readout?.setActive(i===4);document.querySelectorAll<HTMLElement>('[data-mode]').forEach(b=>b.setAttribute('aria-pressed',String(Number(b.dataset.mode)===i)));$('view-title').textContent=['센서 전체','픽셀 배열','픽셀 단면','패키지·회로','셔터·읽기 동작'][i];$('model-label').textContent=['SENSOR PACKAGE','8 × 8 PIXEL ARRAY','2 × 2 PIXEL SECTION','CIRCUIT FLOORPLAN + WIRE-BOND PACKAGE','ROW SELECT → COLUMN CDS → ADC'][i];rebuild();resetCamera();$('package-parts').hidden=!fullPackage();$('package-toolbar').hidden=!fullPackage();if(!fullPackage()&&selectedPart)select(3);}
...
function updateCallout(){
 const anchor=mode!==4&&selectedPart&&packageRig?.anchors.get(selectedPart);const callout=$('part-callout');
...
let last=0;
function animate(t:number){requestAnimationFrame(animate);const dt=Math.min((t-last)/1000,.1);last=t;readout?.tick(dt);if(playing){elapsed+=dt;if(elapsed>3)setStage((stage+1)%4);}particles.visible=playing;particles.children.forEach((p,i)=>{const a=(elapsed/3+i/16)%1;const scale=model.scale.x;const top=groups[0]?.position.y??1;const diode=groups[2]?.position.y??0;if(stage===0)p.position.set(((i%4)-1.5)*.6*scale,(top+2-a*2)*scale,(Math.floor(i/4)-1.5)*.6*scale);else if(stage===1)p.position.set(((i%4)-1.5)*.5*scale,(diode+.3+Math.sin(t*.003+i)*.06)*scale,(Math.floor(i/4)-1.5)*.5*scale);else p.position.set((-2+a*4)*scale,(groups[3].position.y+.2)*scale,(i%4-1.5)*.5*scale);});controls?.update();updateCallout();renderer?.render(scene,camera);}
...
$('export').onclick=async()=>{try{const overlay=readoutVisual?.group,visible=overlay?.visible;let result:ArrayBuffer|{[key:string]:unknown};try{if(overlay)overlay.visible=false;result=await new GLTFExporter().parseAsync(model,{binary:true});}finally{if(overlay)overlay.visible=visible!;}const url=URL.createObjectURL(new Blob([result as ArrayBuffer],{type:'model/gltf-binary'}));const a=document.createElement('a');a.href=url;a.download=`sensor-lab-${['package','array','section','package-circuits','readout'][mode]}.glb`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);$('status').textContent='현재 모델을 GLB 파일로 다운로드했습니다.';}catch{$('status').textContent='모델 내보내기에 실패했습니다. 다시 시도해 주세요.'}};
readout=mountReadout(state=>readoutVisual?.update(state));setMode(4);selectPart('row');experiment();requestAnimationFrame(animate);
...
if(modelContext?.registerTool){
 ...
 const tool={name:'configure_sensor_view',description:'이미지 센서 관찰 범위와 선택 부품, 분해 정도를 설정합니다.',inputSchema:{type:'object',properties:{view:{type:'integer',minimum:0,maximum:4},layer:{type:'integer',minimum:0,maximum:4},separation:{type:'number',minimum:0,maximum:100}},required:['view','layer','separation'],additionalProperties:false},annotations:{readOnlyHint:false,untrustedContentHint:false},execute:async(input:unknown)=>{
 const p=input as {view:number,layer:number,separation:number};
 if(!p||!Number.isInteger(p.view)||p.view<0||p.view>4||!Number.isInteger(p.layer)||p.layer<0||p.layer>4||!Number.isFinite(p.separation)||p.separation<0||p.separation>100)throw new Error('관찰 범위, 부품 또는 분해 값이 유효하지 않습니다.');
