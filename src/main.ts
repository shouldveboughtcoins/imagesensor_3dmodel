...
const layers = [
 ...
 {name:'지지층 · 패키지',en:'SUPPORT & PACKAGE',color:'#8292a3',text:'센서 칩을 지지하고 외부 회로와 연결합니다. 실제 패키지의 형태와 연결 방식은 제품마다 달라집니다.',tip:'이 모델은 특정 제조사 제품의 설계도나 실제 치수를 재현하지 않습니다.'}
];
const stages = [
...
let mode = 1, selected = 0, separation = .48, stage = 0, playing = false, elapsed = 0;
...
try {
 ...
 controls.maxPolarAngle=Math.PI*.86; controls.autoRotateSpeed=.7;
...
function rebuild(){
 ...
 model.clear(); groups=layers.map((_,i)=>{const g=new THREE.Group();g.userData.layer=i;model.add(g);return g;});
 const n=mode===2?2:8, step=mode===2?1.55:.66, width=n*step;
 box(width+.9,.42,width+.9,'#263948',4);
 box(width+.3,.22,width+.3,'#455968',3);
 ...
 if(mode===0){
  for(let k=0;k<12;k++)for(const sign of [-1,1])box(.24,.13,.62,'#bea66e',4,(k-5.5)*.49,sign*(width/2+.57),0);
 }
 model.scale.setScalar(mode===0?.82:mode===2?1.35:1);
 positionLayers(); highlight();
}
function positionLayers(){
 const base=[1.04,.86,.51,.12,-.25];
 groups.forEach((g,i)=>{g.position.y=base[i]+separation*(4-i)*.9-1;g.visible=visibility[i];});
}
function highlight(){groups.forEach((g,i)=>g.traverse(o=>{if(o instanceof THREE.Mesh && o.material instanceof THREE.MeshStandardMaterial){o.material.emissive.set(i===selected?'#35502d':'#000000');o.material.emissiveIntensity=i===selected?.24:0;}}));}
function select(i:number){selected=i;const l=layers[i];$('detail-number').textContent=String(i+1).padStart(2,'0');$('detail-en').textContent=l.en;$('detail-name').textContent=l.name;$('detail-text').textContent=l.text;$('detail-tip').textContent=l.tip;document.querySelectorAll<HTMLElement>('[data-layer]').forEach(b=>b.setAttribute('aria-pressed',String(Number(b.dataset.layer)===i)));highlight();}
function resetCamera(){camera.position.set(mode===2?8:10,10,mode===2?10:13);controls?.target.set(0,.5,0);controls?.update();}
function setMode(i:number){mode=i;document.querySelectorAll<HTMLElement>('[data-mode]').forEach(b=>b.setAttribute('aria-pressed',String(Number(b.dataset.mode)===i)));$('view-title').textContent=['센서 전체','픽셀 배열','픽셀 단면'][i];$('model-label').textContent=['SENSOR PACKAGE','8 × 8 PIXEL ARRAY','2 × 2 PIXEL SECTION'][i];rebuild();resetCamera();}
...
$('export').onclick=async()=>{try{const result=await new GLTFExporter().parseAsync(model,{binary:true});const url=URL.createObjectURL(new Blob([result as ArrayBuffer],{type:'model/gltf-binary'}));const a=document.createElement('a');a.href=url;a.download=`sensor-lab-${['package','array','section'][mode]}.glb`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);$('status').textContent='현재 모델을 GLB 파일로 다운로드했습니다.';}catch{$('status').textContent='모델 내보내기에 실패했습니다. 다시 시도해 주세요.'}};
rebuild();select(0);experiment();resetCamera();requestAnimationFrame(animate);
...
if(modelContext?.registerTool){
 ...
 const tool={name:'configure_sensor_view',description:'이미지 센서 관찰 범위와 선택 부품, 분해 정도를 설정합니다.',inputSchema:{type:'object',properties:{view:{type:'integer',minimum:0,maximum:2},layer:{type:'integer',minimum:0,maximum:4},separation:{type:'number',minimum:0,maximum:100}},required:['view','layer','separation'],additionalProperties:false},annotations:{readOnlyHint:false,untrustedContentHint:false},execute:async(input:unknown)=>{
 const p=input as {view:number,layer:number,separation:number};
 if(!p||!Number.isInteger(p.view)||p.view<0||p.view>2||!Number.isInteger(p.layer)||p.layer<0||p.layer>4||!Number.isFinite(p.separation)||p.separation<0||p.separation>100)throw new Error('관찰 범위, 부품 또는 분해 값이 유효하지 않습니다.');
